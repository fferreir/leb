...
2. Go to your repository at [fferreir/leb](https://github.com/fferreir/leb).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `leb.fmvz.usp.br` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | leb.fmvz.usp.br | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/fferreir/leb/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/fferreir/leb/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/fferreir/leb-backend`) and Frontend (`ghcr.io/fferreir/leb-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of leb.fmvz.usp.br](https://github.com/fferreir/leb/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/leb.fmvz.usp.br.yml` stack.
